...
export class Tab1Page {
  ...
  constructor(private modalController:ModalController, private toastController: ToastController, private firestore: AngularFirestore, private alertController: AlertController,) {
  }
    ...
    createOrder: (data: any, actions: any) => {
      ...
      return actions.order.create({
        purchase_units: [
          {
            amount: {
              value: String(this.total), // Ajusta esto según el monto real del boleto
            },
          },
        ],
      });
        ...
        if (docSnapshot && docSnapshot.exists) {
          ...
          if (cooperativa) {
            const transactionData: Transaccion = {
              fecha: new Date(), // Fecha actual del pago
              precio: String(this.total), // Precio obtenido de la cooperativa
              rutaId: 't0mLWioavjJciKNS3HC1', // Aquí pones el ID de la ruta correspondiente
              userId: String(this.userName), // ID del usuario que realiza el pago
